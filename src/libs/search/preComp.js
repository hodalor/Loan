const _filterPreCompCases = async (data) => {
  ...
  if (type === "Advance Staff") {
    filteredData = dataToFilter.filter(
      (item) => item.preCollOfficer === filter
    );
  }

  if (type === "phone") {
    let customer = customers.find((item) => item.phone === filter);

    filteredData = customer
      ? dataToFilter.filter((item) => item.userId === customer.userId)
      : [];
  }
...
export default _filterPreCompCases;
