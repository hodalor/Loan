const _filterAssPreCases = async (data) => {
  ...
  if (type === "Case Status") {
    if (filter === "caseNotRecorded") {
      filteredData = dataToFilter.filter(
        (item) => item.preCollCallRecords.length === 0
      );

    }

    if (filter === "recordedCase") {
      filteredData = dataToFilter.filter(
        (item) => item.preCollCallRecords.length > 0
      );
...
export default _filterAssPreCases;
