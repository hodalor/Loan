  ...
  return (
    <header className="sticky top-0 z-20 border-b border-slate-200 bg-white/90 backdrop-blur">
      <div className="flex items-center justify-between gap-4 px-4 py-4 sm:px-6 lg:px-8">
        ...
        <div className="flex items-center gap-3">
          <div className="hidden items-center gap-2 rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-sm text-slate-600 md:flex">
            <i className="fa fa-clock-o text-sm" />
            <span>Auto logout after 5 minutes idle</span>
          </div>

          {showNotifications ? (
            <div className="relative inline-flex h-11 w-11 items-center justify-center rounded-xl border border-slate-200 text-slate-700">
          ...
          <div className="relative" ref={profileMenuRef}>
            <button
              ...
              className="inline-flex h-11 w-11 items-center justify-center rounded-xl border border-slate-200 bg-white text-sm font-semibold text-slate-700 transition hover:bg-slate-50"
            >
              <span className="inline-flex h-[30px] w-[30px] items-center justify-center rounded-full bg-slate-800 text-[13px] font-semibold text-white">
                {(user?.userName || "U").slice(0, 1)}
              </span>
            </button>

            <div
              className={`absolute right-0 top-[calc(100%+10px)] z-30 min-w-[220px] rounded-3xl border border-slate-200 bg-white shadow-[0_16px_40px_rgba(15,23,42,0.12)] transition ${
                menuOpen
