...
export default function PreModalContent(props) {
  ...
  const assignLoading = isActionLoading("precollection-assign");
  const reassignLoading = isActionLoading("precollection-reassign");
  ...
  const rows = React.useMemo(
    () =>
      admins
        .filter((item) => item.role === "pre-team-lead" || item.role === "pre-personel")
        .map((review) => {
          const assigned =
          ...
          render: (row) => (
            <button
              ...
              onClick={async (event) => {
                ...
                try {
                  await _handleReAssignPreCase(row);
                } finally {
                  setPendingUserId("");
                }
              }}
    ...
    <div className="flex w-full max-w-[680px] flex-col p-3 sm:p-3.5">
      ...
      {personnelLoading ? (
        <div className="mb-2 rounded-xl bg-slate-50 px-3 py-2 text-xs text-slate-500">
          <div className="flex items-center gap-2">
            <DefaultLoader />
            <span>Loading precollection personnel...</span>
          </div>
        </div>
      ) : null}
      ...
      <div className="max-h-[68vh] overflow-auto">
        <SimpleDataTable
          ...
          loadingMessage="Loading precollection personnel..."
          emptyMessage="No precollection personnel found."
          onRowClick={
            isReassignMode
              ? undefined
