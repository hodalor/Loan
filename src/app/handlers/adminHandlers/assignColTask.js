...
const _assignColTask = async (data) => {
  ...
  await Loans.bulkWrite(
    assignments.map((assignment) => ({
      updateOne: {
        ...
        update: {
          $set: {
            collofficer: assignment.userName,
          },
        },
      },
    }))
...
const _reAssignColTask = async (data) => {
  const adminCases = data.casesSelected.flat(1);
  const ids = adminCases.map((cas) => cas.ID);
  const personnelList = Array.isArray(data.personnelList)
    ? data.personnelList
  ...
  const oldAdminNames = [
    ...new Set(
      adminCases
        .map((item) => String(item.collofficer || "").trim())
        .filter(Boolean)
    ),
  ];
...
module.exports = { _assignColTask, _reAssignColTask };
